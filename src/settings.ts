...
const blank = (): Settings => ({ telegram: { botToken: '', chatId: '' }, rpc: { ethereum: '', robinhood: '', bsc: '', solana: '' }, solana: { jupiterApiKey: '', priorityFee: '', historyTxs: '' }, api: { uniswapUrl: '' }, gmgn: { apiKey: '' }, squeeze: {} })
...
export const secretValues = () => [store.telegram.botToken, store.rpc.ethereum, store.rpc.robinhood, store.rpc.bsc, store.rpc.solana, store.solana.jupiterApiKey, store.gmgn.apiKey].filter((v) => v.length >= 8)
