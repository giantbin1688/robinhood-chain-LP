// 网页"设置"页存的东西（settings.json，已 git 忽略）：Telegram。
...
const blank = (): Settings => ({ telegram: { botToken: '', chatId: '' } })
const store: Settings = (() => { const b = blank(); if (!existsSync(FILE)) return b; try { const j = JSON.parse(readFileSync(FILE, 'utf8')); return { telegram: { ...b.telegram, ...j.telegram } } } catch { return b } })()
...
export const secretValues = () => [store.telegram.botToken].filter((v) => v.length >= 8)
