// 网页"设置"页存的东西（settings.json，已 git 忽略）：Telegram、FomoScan key。
...
const blank = (): Settings => ({ telegram: { botToken: '', chatId: '' }, fomoscan: { key: '' } })
const store: Settings = (() => { const b = blank(); if (!existsSync(FILE)) return b; try { const j = JSON.parse(readFileSync(FILE, 'utf8')); return { telegram: { ...b.telegram, ...j.telegram }, fomoscan: { ...b.fomoscan, ...j.fomoscan } } } catch { return b } })()
...
export const secretValues = () => [store.telegram.botToken, store.fomoscan.key].filter((v) => v.length >= 8)
