...
const files: Record<string, [string, string]> = { launch: ['./cli.ts', './sol/cli.ts'], exit: ['./exit.ts', './sol/exit.ts'], watch: ['./monitor.ts', './sol/monitor.ts'], scan: ['./scan.ts', './scan.ts'] }
if (!files[cmd]) { console.error('用法: run.ts launch|exit|watch|scan [参数…]'); process.exit(1) }
