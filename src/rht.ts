// rhtrenches.com：第三方、只读、免登录的 fomo.family 交易者链上成交流（Robinhood 链，147 个头部交易者），公开 API + WebSocket。
...
async function getJson<T>(path: string): Promise<T> {
  ...
  if (!r.ok) throw new Error(`rhtrenches ${path.split('?')[0]} HTTP ${r.status}`)
...
function ingest(rows: Fill[], live: boolean) {
  for (const f of [...rows].sort((a, b) => a.id - b.id)) {
    ...
    try { onFill(f, live) } catch (e: any) { log(`rhtrenches 成交 #${f.id} 处理失败: ${String(e?.message).slice(0, 100)}`) }
  }
}
...
export async function start(o: { onFill: typeof onFill; onStatus: typeof onStatus }) {
  if (started) return
