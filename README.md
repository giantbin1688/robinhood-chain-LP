...
- **信号**页：盯 [fomo.family](https://fomo.family) 上交易者的钱包，抓到买入 / 卖出就记一条信号并提醒，买入的币顺手做一遍**安全检查**。fomo.family 没有公开接口（后端要登录态的 Privy token），所以直接盯链：fomo 的交易是 ERC-4337 UserOp（bundler 代发，钱包本身不发交易），服务每 2 秒查一次两个方向的 ERC-20 `Transfer` 日志——代币从 fomo 路由 `0xb92f…4fff4f` 转进钱包 = **买入**（钱由 fomo 资金池垫付，同一笔里 USDG / WETH 最大的一段转账就是本金），代币从钱包转给路由 = **卖出**（USDG 回 fomo 金库 `0x4cd0…8bc31`，不回钱包），USDG / WETH 和 fomo 之间的进出 = **资金**，来源不是路由的转入 = **空投**（别人打过来的碎币，fomo 上很常见，默认不显示、不提醒）。**第二个来源是 [rhtrenches.com](https://rhtrenches.com)**（第三方、只读、免登录的 fomo 头部交易者成交流，公开 API + WebSocket，147 个钱包）：它能分出**别人买了塞进钱包**的假买入（`planted` / `transferred` / `spoofed` / `airdropped`）和本人买入。这两种在链上一模一样（都是中继代付、代币经 fomo 路由进钱包，只有付款方不同），我们自己分不出来；典型的塞币是十几美元一笔、批量打进大 V 钱包，fomo 个人页的交易记录里不会出现。服务启动先同步它最近 400 笔，再连 WebSocket（断了退回 5 秒轮询）；同一笔交易两边都抓到就合并（每行标"链上 / rht / 链上+rht"），它的真假标记优先，标了塞的记成"塞币/空投"不提醒；链上先抓到的买入在它在线时等 8 秒再提醒。它还提供 handle 到钱包的名单，所以添加交易者只填 handle 就行。新加钱包先补扫最近 1 小时（Alchemy 节点约 40 秒），之后实时（0.1 秒出块，信号比 fomo 自己的 feed 还快）；已见过的交易不重复记，信号存在 `signals.json`（最多 1000 条）。添加交易者填 **FOMO handle**（先查 rhtrenches 的名单，再查 FomoScan（付费，可选），都没有就把 `fomo.family/profile/<handle>` 头像旁的钱包地址贴到第二格）；头像用 fomo 公开的分享卡片图裁出来；每个交易者可以 ON/OFF、静音（只记录不提醒）、移除。信号流按类型 / 安全结论筛选、按交易者或代币搜索，每行给出金额（美元和枚数、成交单价）、说明、安全检查结论和交易链接，买入行有"进场"（跳到进场页、代币地址填好）和"重查"。**安全检查**（排队做、间隔 2.5 秒，免得把 GeckoTerminal 打到 429）只用链上和 GeckoTerminal 能查到的东西（GoPlus 之类不支持 Robinhood Chain）：所有池里流动性最大的那个（< $2k 风险、< $10k 注意）、池龄（< 1 小时注意）、24h 涨幅（> 500% 注意）、24h 买卖人数（很多人买没人卖过注意）、流动性占 FDV 比例（< 1% 注意）、合约是不是可升级代理、owner 放没放弃、字节码里有没有 `mint` / `pause` / 黑名单函数（有且 owner 还在 = 风险）、代币能不能从池子转出（`eth_call` 模拟 PoolManager 转账失败 = 貔貅，风险）、100 美元买进再卖出的往返损耗（> 20% 注意、> 50% 风险，报价失败也标注意）。结论 **通过 / 注意 / 风险** 只是筛选参考，鼠标悬停看全部理由和数据。提醒：页面 toast + 浏览器桌面通知（点"开启桌面通知"授权一次，页面不在前台时才弹）+ 可选 Telegram（`.env` 里 `TG_BOT_TOKEN` / `TG_CHAT_ID`，买卖各一条，安全检查不通过再补一条）。目前只有 Robinhood Chain 核对过 fomo 的合约地址，BSC 上加交易者会被拒绝。
- **设置**页：Telegram（bot token + chat id，"发测试消息"验证）和 FomoScan key（付费第三方，按 handle 查钱包），存在 `settings.json`（已 gitignore；同名项优先于 `.env`；接口只回"配没配 + 末 4 位"，原值不出服务）。fomo.family 本身的账号接入试过了：它的后端在 Cloudflare 机器人拦截后面，脚本请求即使 token 有效也被拒（HTTP 430），所以没有这一项。
...
| `src/signals.ts` `src/rht.ts` | 信号：盯 FOMO 交易者钱包的链上买卖 + rhtrenches.com 的成交流（识别塞币）、代币安全检查、Telegram 推送；名单和信号存 `signals.json` |
| `src/settings.ts` | 设置页存的 Telegram / FomoScan（`settings.json`） |
