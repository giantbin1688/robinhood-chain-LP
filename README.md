...
- 左侧是导航栏（进场 / 仓位 / 信号 / 盈亏日历 / 任务 / 设置，仓位和任务后面是数量角标、信号后面是没看过的新信号数，底部标当前链和协议），顶栏是钱包 / 余额 / 原生币价格 / 链四个信息块，右上角一排**状态灯**（节点：绿 = Alchemy、黄 = 公共节点；钱包：有没有私钥；Uniswap / OKX：聚合器 key 配没配；fomo：fomo 账号登没登上（灰 = 没配，红 = token 失效）；事件流：网页和服务的实时连接；信号：FOMO 钱包监控在不在跑（黄 = 补扫中，红 = 读链出错）；任务：有任务在跑时蓝色闪烁；鼠标悬停看说明）和 ☾ / ☀ **明亮 / 黑暗模式**切换（第一次打开跟随系统，切过之后记在浏览器里）。顶栏的下拉框切换 **链**（Robinhood / BSC），进场页表单第一行选 **协议**（BSC 上：PancakeSwap Infinity / PancakeSwap v3 / Uniswap v4；Robinhood 只有 Uniswap v4，这一行不显示；每条链各自记住上次选的协议），仓位、日历、进场都按所选链显示，任务列表是全部链的（任务名前缀 `[BSC]` / `[RHC]`）；钱包余额、计价币（USDG / USDT）、原生币（ETH / BNB）随之切换，选择记在浏览器里。
...
- **信号**页：盯 [fomo.family](https://fomo.family) 上交易者的钱包，抓到买入 / 卖出就记一条信号并提醒，买入的币顺手做一遍**安全检查**。fomo.family 没有公开接口（后端要登录态的 Privy token），所以直接盯链：fomo 的交易是 ERC-4337 UserOp（bundler 代发，钱包本身不发交易），服务每 2 秒查一次两个方向的 ERC-20 `Transfer` 日志——代币从 fomo 路由 `0xb92f…4fff4f` 转进钱包 = **买入**（钱由 fomo 资金池垫付，同一笔里 USDG / WETH 最大的一段转账就是本金），代币从钱包转给路由 = **卖出**（USDG 回 fomo 金库 `0x4cd0…8bc31`，不回钱包），USDG / WETH 和 fomo 之间的进出 = **资金**，来源不是路由的转入 = **空投**（别人打过来的碎币，fomo 上很常见，默认不显示、不提醒）。新加钱包先补扫最近 1 小时（Alchemy 节点约 40 秒），之后实时（0.1 秒出块，信号比 fomo 自己的 feed 还快）；已见过的交易不重复记，信号存在 `signals.json`（最多 1000 条）。添加交易者要填 **FOMO handle** 和**钱包地址**；设置页配了 fomo 账号就可以只填 handle（钱包、头像、显示名从 fomo 拿），或配 FomoScan key 按 handle 查（付费第三方）；都没配就手填地址；每个交易者可以 ON/OFF、静音（只记录不提醒）、移除。信号流按类型 / 安全结论筛选、按交易者或代币搜索，每行给出金额（美元和枚数、成交单价）、说明、安全检查结论和交易链接，买入行有"进场"（跳到进场页、代币地址填好）和"重查"。**安全检查**只用链上和 GeckoTerminal 能查到的东西（GoPlus 之类不支持 Robinhood Chain）：所有池里流动性最大的那个（< $2k 风险、< $10k 注意）、池龄（< 1 小时注意）、24h 涨幅（> 500% 注意）、24h 买卖人数（很多人买没人卖过注意）、流动性占 FDV 比例（< 1% 注意）、合约是不是可升级代理、owner 放没放弃、字节码里有没有 `mint` / `pause` / 黑名单函数（有且 owner 还在 = 风险）、代币能不能从池子转出（`eth_call` 模拟 PoolManager 转账失败 = 貔貅，风险）、100 美元买进再卖出的往返损耗（> 20% 注意、> 50% 风险，报价失败也标注意）。结论 **通过 / 注意 / 风险** 只是筛选参考，鼠标悬停看全部理由和数据。提醒：页面 toast + 浏览器桌面通知（点"开启桌面通知"授权一次，页面不在前台时才弹）+ 可选 Telegram（`.env` 里 `TG_BOT_TOKEN` / `TG_CHAT_ID`，买卖各一条，安全检查不通过再补一条）。目前只有 Robinhood Chain 核对过 fomo 的合约地址，BSC 上加交易者会被拒绝。
- **设置**页：三样东西，存在 `settings.json`（fomo 账号这一项实测**不可靠**：fomo 后端前面有 Cloudflare 机器人拦截，脚本请求即使 token 有效也大多回 430，留着只为偶尔一试；信号功能本身靠链上，不依赖它）（已 gitignore；同名项优先于 `.env`；接口只回"配没配 + 末 4 位"，原值不出服务）。**fomo 账号**：fomo.family 没有公开 API，后端认的是 Privy 的登录 token——用**无痕窗口**登录一次 fomo，F12 → Application → Local storage → fomo.family 里复制 `privy:token`（access token，1 小时过期）和 `privy:refresh_token`（refresh token 只在 localStorage，Cookies 里只有 access token），贴进来"保存并验证"，然后关掉无痕窗口。服务在过期前 10 分钟拿这一对去 `auth.privy.io` 换新的并存回来；refresh token 是一次性轮换的，所以**不能**和你日常浏览器共用同一次登录（两边轮流刷新会互相踢下线），无痕窗口单独登一次就没这个问题。登上以后：添加交易者只填 handle、头像用 fomo 的原图；token 失效（fomo 那边退出登录、长期没续上）顶栏 fomo 灯变红，重贴一次即可，链上监控不受影响。**Telegram**：bot token + chat id，"发测试消息"验证。**FomoScan**：付费第三方的 API key，配了 fomo 账号就用不上。
...
| `FOMOSCAN_KEY` | 可选。信号页添加交易者时只填 handle、自动查钱包（https://www.fomoscan.sh，付费第三方）。也可以在设置页填；配了 fomo 账号就用不上 |
...
| `src/signals.ts` | 信号：盯 FOMO 交易者钱包的链上买卖、代币安全检查、Telegram 推送；名单和信号存 `signals.json` |
| `src/fomo.ts` `src/settings.ts` | fomo 账号接入（Privy token 续期、按 handle 查用户 / 拉历史交易）；设置页存的 token / Telegram / FomoScan（`settings.json`） |
