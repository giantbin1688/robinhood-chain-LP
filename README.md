...
- **仓位**页：钱包名下所有 USDG 池的仓位，显示区间、现价、持仓、折合价值、未领手续费、**uPNL** 和**持仓时间**，以及在不在区间内。uPNL = 现在的持仓价值 + 未领手续费 − 入场价值；入场价值是 mint 交易里存入池子的两种币按 mint 当时的池价折算（不含进场换币的手续费/滑点），持仓时间从 mint 区块算起，这两项对任何仓位（包括不是本工具建的）都能算出来。点 id 展开**该仓位所在池子的流动性分布图**（蓝 = 现价下方的 USDG 侧，绿 = 现价上方的代币侧，淡色 = 你的区间外，悬停看每段数量）。每个仓位可"领取"手续费（只领手续费，本金不动，等于 `npm run exit -- --position <id> --collect`）、"开始监控"（各仓位独立监控，跳出区间自动撤退）、"撤退"（只撤这一个、只卖撤出来的币，点了会弹确认）或"撤全部 X"（该代币全部仓位 + 卖光钱包里的币）。正在监控的仓位会标出是哪个任务在盯。
