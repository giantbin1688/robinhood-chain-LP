...
日历统计的是 LP 仓位的存入、撤出本金和手续费估值，再扣掉建仓 / 撤仓 / 领取交易的 gas（同一笔交易里的几个仓位均分，按当前原生币价折算；Solana 是交易费，租金退还不算），不等同于整次操作的现金收支（换币、授权的 gas 和钱包闲置代币不含）。遇到 Uniswap 极限池价时，先按区块内事件顺序还原操作时价格；仍为极限价的撤仓，只在能核对紧随其后的同钱包卖币交易时使用实际成交价，并显示交易链接。没有可靠价格则暂不计算，不能把撤出的代币按零价记账。
...
| `LP_SHAPE` `LP_LAYERS` | `spot` `3` | 流动性形状（`spot` / `curve` / `bidask` / `squeeze`）/ `curve` 的层数、`bidask` 每侧段数（2~8） |
| `SQ_*` | 见 `params.env` | `squeeze` 形状的闸门与形状参数（下文"形状"一节） |
...
`--shape squeeze` 不看 `RANGE` / `PRICE_RANGE` / `LP_LAYERS`，区间和资金份额由 GMGN 数据按公式推导（`src/strategy.ts`），适合**窄幅震荡 + 高换手**、准备在两侧接挤压（squeeze）单的场景。所有 `SQ_*` 参数也能在设置页 GMGN 卡片里填，填了优先于 `params.env`。需要在设置页填 GMGN API key（gmgn.ai/ai 免费申请，只读，不能交易；也可放 `.env` 的 `GMGN_API_KEY`），只支持 Robinhood / BSC / Ethereum。

所有尺度都从**这个币自己最近 1 小时的走势**和**目标池的费率 / 间距**里取，没有写死的百分比；参数只剩倍数和比例（`SQ_*`）。五条闸门也是和自己比，**默认只提示不拦**（`SQ_GATE_BLOCK=0`）：不管过没过，形状都按数据算出来照常建仓，闸门结果打在日志和计划卡片里供你判断；设 `SQ_GATE_BLOCK=1` 才禁止不过闸门的真实进场（演练永远放行）。

| 闸门 | 默认（`params.env`） | 指标 |
|---|---|---|
| 活跃 | `SQ_HOT_MIN=1` | 当前 5 分钟成交 ≥ 过去 1 小时平均每 5 分钟成交 × 1（成交没在退潮）。同时显示"相当于池内 USDG 的几倍"，池内 USDG 从链上按 tick 求和，不用 GMGN 会在 v3/WETH、RWA 计价池之间跳的"主池" |
| 收敛 | `SQ_COMPRESS_MAX=0.5` | 观察窗口（`SQ_WINDOW_MIN=10` 分钟）振幅 ≤ 1 小时振幅 × 0.5（在收窄，像横盘） |
| 漂移 | `SQ_DRIFT_SIGMAS=2` | \|5 分钟漂移\| ≤ 2 × σ₁ₘ√5（在自身波动的正常范围内） |
| 深度 | `SQ_BUDGET_SHARE_MAX=5` | 预算 ≤ 目标池内 USDG 的 5% |
| 安全 | `SQ_TOP10_MAX=40` | 非蜜罐、24h 有卖出、卖税 < 10%、前 10 持仓 ≤ 40% |

形状 = 现价附近 **2 层 Curve 核心** + 两侧 **各 2 段 Bid-Ask 翼**（远段厚），同一笔交易创建：

```
核心半宽 w = max(观察窗口 (最高 − 最低) / 2, 费率 × SQ_CORE_FLOOR_FEES, 2 个 tick 间距)     2.5% 池横盘 ±1% 时 → w = 5%
翼外沿   上 = max(1 小时最高 − 1, SQ_WING_MIN_MULT · w) · (1 + 0.5s)，下 = max(1 − 1 小时最低, SQ_WING_MIN_MULT · w) · (1 − 0.5s)
偏斜     s = (买 − 卖) / 5 分钟成交额 ∈ [−1, 1]
核心份额 c = 1 小时里收盘价落在 [P(1−w), P(1+w)] 内的时间占比，不低于 SQ_CORE_SHARE_MIN；上翼 (1−c)(1+s)/2，下翼 (1−c)(1−s)/2；不到 5% 的翼并入另一侧
```

`npm run scan -- --chain robinhood --token <地址> --usdg 500 [--pool <池id> | --fee 4]` 拉数据、读目标池储备，打印指标、闸门和形状，只读链不建仓；目标池按进场的 auto 规则选（同费率优先，否则最活跃），网页进场页选 Squeeze 后点"扫描 GMGN 数据"是同一件事。监控 squeeze 组时区间规则按**整组并集**判断（核心那一小段进出不算跳出），并每分钟问一次 GMGN 活跃度（当前 5 分钟成交 ÷ 1 小时均值），连续 `SQ_EXIT_CONFIRM=3` 分钟低于 `SQ_EXIT_HEAT=0.25` 就整组撤退；止损规则照旧。
...
npm run watch -- --position 2070306,2070307,2070308 --stop-loss 30 [--entry 500]   # 整组按本金止损：亏 30% 就撤；本金默认从流水读，没 Alchemy 时手填
...
- 止损按整组算：Curve / Bid-Ask 的几段是一起进场的，只看一段会把组打散。盈亏 = 现值 + 未领手续费 + 已领手续费 + 已撤本金 − 存入本金，连续 `WATCH_CONFIRM` 次低于 −止损% 才撤。**开了止损就不再看跳出区间**（否则 Bid-Ask 贴现价那段一进一出就把整组撤了）。`launch --watch` 的本金用本次预算，仓位页开的监控从 Alchemy 流水读，读不到要手填本金，否则拒绝启动（不会默默跑成没止损）。
