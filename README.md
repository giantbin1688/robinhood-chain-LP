...
每 `WATCH_INTERVAL` 秒读一次池子的 tick，判断主要仓位是否在区间内（忽略过渡仓位和粉尘）。连续 `WATCH_CONFIRM` 次跳出区间就自动执行撤退：撤仓 + 领手续费 + 代币卖成 USDG，然后进程结束。每分钟核对一次仓位是否还在：**你手动撤掉了，监控自动停止**。日志只在状态变化或每 5 分钟打一行，不刷屏；RPC 偶发出错会继续重试，连续 30 次失败才退出。Ctrl+C 随时停止，仓位不受影响。
